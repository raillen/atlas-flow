...
| CI green on supported platforms | **PARTIAL** | The repository declares Ubuntu, macOS and Windows jobs; local canonical gates pass Python (311 tests), TypeScript, JavaScript (56 tests), lint and desktop checks. Hosted macOS/Windows results were not available in this audit. |
...
| Installation tests | **PARTIAL** | `sh scripts/package_smoke.sh` builds and verifies the `.deb` and the AppImage — the latter unpacked and checked for its executable — plus SBOM, checksums and signature. Windows and macOS installers are still unbuilt. |
...
| No DOM/screen-reader audit | Partly closed — axe-core/DOM suite passes 10 tests; no screen-reader walkthrough was evidenced. |
...
| No hosted result for Windows/macOS | Open — the matrix exists; it reports on the next push |
